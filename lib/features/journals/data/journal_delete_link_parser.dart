...
String? extractJournalDeleteLink(
  dom.Document document,
  String journalId,
) {
  final directDeleteHref = document
      .querySelector('a[href*="/controls/deletejournal/$journalId/"]')
      ?.attributes['href'];
  ...
  for (final anchor in candidateAnchors) {
    ...
    if (href != null) {
      ...
      if (hrefMatch?.group(1) == journalId) {
        return href;
      }
    }
...
String? _extractDeleteLinkFromOnClick(
  ...
  final escapedNumber = RegExp.escape(journalId);
  final match = RegExp(
